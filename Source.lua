if debugX then
	warn('Initialising Godhub')
end

local requestsDisabled = getgenv and getgenv().DISABLE_Godhub_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.671"
local GodhubFolder = "Godhub"
local ConfigurationFolder = GodhubFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		GodhubOpen = {Type = 'bind', Value = 'K', Name = 'Godhub Keybind'},
		-- buildwarnings
		-- Godhubprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(GodhubFolder) then
				if isfile and isfile(GodhubFolder..'/settings'..ConfigurationExtension) then
					file = readfile(GodhubFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"GodhubOpen":{"Value":"K","Type":"bind","Name":"Godhub Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Godhub Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Godhub had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("Godhub", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify Godhub developers at sirius.menu/discord.")
				end
			end)
...
local Godhub = useStudio and script.Parent:FindFirstChild('Godhub') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if Godhub:FindFirstChild('Build') and Godhub.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Godhub | Build Mismatch')
		print('Godhub may encounter issues as you are running an incompatible interface version ('.. ((Godhub:FindFirstChild('Build') and Godhub.Build.Value) or 'No Build') ..').\n\nThis version of Godhub is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Godhub = Godhub, useStudio and script.Parent:FindFirstChild('Godhub') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

Godhub.Enabled = false

if gethui then
	Godhub.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(Godhub)
	Godhub.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Godhub.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Godhub.Name and Interface ~= Godhub then
			Interface.Enabled = false
			Interface.Name = "Godhub-Old"
		end
	end
elseif not useStudio then
...
if Godhub.AbsoluteSize.X < minSize.X and Godhub.AbsoluteSize.Y < minSize.Y then
...
local Main = Godhub.Main
local MPrompt = Godhub:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Godhub:FindFirstChild('Drag')
...
Godhub.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadstring(game:HttpGet('https://raw.githubusercontent.com/SiriusSoftwareLtd/Godhub/refs/heads/main/icons.lua'))()
...
local Notifications = Godhub.Notifications

local SelectedTheme = GodhubLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = GodhubLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Godhub.Main.BackgroundColor3 = SelectedTheme.Background
	Godhub.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Godhub.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Godhub.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Godhub.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Godhub.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Godhub.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Godhub.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Godhub.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Godhub:GetDescendants()) do
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Godhub had an issue decoding the configuration file, please try delete the file and reopen Godhub.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(GodhubLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Godhub | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--GodhubLibrary:Notify({Title = "Godhub Flags", Content = "Godhub was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(GodhubLibrary.Flags) do
...
function GodhubLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Godhub | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			GodhubLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show Godhub'.", Duration = 7, Image = 4400697855})
		else
			GodhubLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.GodhubOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(GodhubFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Godhub Settings', 0, true)

	if TabList['Godhub Settings'] then
		TabList['Godhub Settings'].LayoutOrder = 1000
	end

	if Elements['Godhub Settings'] then
		Elements['Godhub Settings'].LayoutOrder = 1000
	end
...
function GodhubLibrary:CreateWindow(Settings)
	if Godhub:FindFirstChild('Loading') then
		if getgenv and not getgenv().GodhubCached then
			Godhub.Enabled = true
			Godhub.Loading.Visible = true

			task.wait(1.4)
			Godhub.Loading.Visible = false
		end
	end

	if getgenv then getgenv().GodhubCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				GodhubLibrary:Notify({Title = 'Build Mismatch', Content = 'Godhub may encounter issues as you are running an incompatible interface version ('.. ((Godhub:FindFirstChild('Build') and Godhub.Build.Value) or 'No Build') ..').\n\nThis version of Godhub is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(GodhubFolder) then
		makefolder(GodhubFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Godhub"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Godhub Interface Suite" then
		LoadingFrame.Version.Text = "Godhub UI"
	end
	...
	if not Settings.DisableGodhubPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				GodhubLibrary:Notify({
					Title = "Godhub Interface",
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(GodhubFolder.."/Discord Invites") then
			makefolder(GodhubFolder.."/Discord Invites")
		end

		if isfile and not isfile(GodhubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(GodhubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Godhub RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(GodhubFolder.."/Key System") then
			makefolder(GodhubFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Godhub | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Godhub specific development.')
				end
			end
		end
		...
		if isfile and isfile(GodhubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(GodhubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(GodhubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						GodhubLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Godhub | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Godhub specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= GodhubLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					GodhubLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Godhub.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Godhub | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Godhub specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					GodhubLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Godhub | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Godhub specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					GodhubLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Godhub | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Godhub specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					GodhubLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= GodhubLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Godhub | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Godhub specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						GodhubLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Godhub | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Godhub specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					GodhubLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			GodhubLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			GodhubLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Godhub had an issue creating settings.') end
...
function GodhubLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function GodhubLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function GodhubLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Godhub Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.GodhubOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function GodhubLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				GodhubLibrary:Notify({Title = "Godhub Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			GodhubLibrary:Notify({Title = "Godhub Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Godhub Configurations Error | '..tostring(result))
			GodhubLibrary:Notify({Title = "Godhub Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = GodhubLibrary:CreateWindow({
		Name = "Godhub Example Window",
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using Godhub may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Godhub to get the key from
	...
	--GodhubLibrary:Notify({Title = "Godhub Interface", Content = "Welcome to Godhub. These - are the brand new notification design for Godhub, with custom sizing and Godhub calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(GodhubLibrary.Theme) do
...
task.delay(4, function()
	GodhubLibrary.LoadConfiguration()
